...
#dim = index.d
#last_rows: Dict[int, sqlite3.Row] = {}
...
def search(conn, model, index, dim, q, k=k, min_score=min_score, fetch_k=fetch_k, book_ids=book_ids):
    try:
        # DEBUG: Print what we received
        print(f"DEBUG: q={q}, k={k}, min_score={min_score}, fetch_k={fetch_k}", file=sys.stderr)
        ...
        # DEBUG: Print the raw scores from FAISS
        print(f"DEBUG: First 10 raw FAISS scores: {scores[:10]}", file=sys.stderr)
        ...
        for cid, score in zip(ids, scores):
            ...
            if score < min_score:
                print(f"DEBUG: Filtered out score {score} (below {min_score})", file=sys.stderr)
                continue
...
def search1(conn, model, index, dim, q=q, k=k, min_score=min_score, fetch_k=fetch_k, book_ids=book_ids):
    try:
        qv = model.encode([q], normalize_embeddings=True, convert_to_numpy=True).astype("float32")
        if qv.shape[1] != dim:
            raise RuntimeError("Model/index dimension mismatch.")
        ...
        #     print(f"[{rank}] cos={score:.3f} book_id={row['book_id']} chunk={row['chunk_index']}")
...
ap.add_argument("--min_score", type=float, default=-1.0, help="Min similarity score to return (default: -1.0)")
...
# overwrite globals from args (keeping your structure)
q = args.q
k = args.k
...
results = search(conn, model, index, dim, q=q, k=k, min_score=min_score, fetch_k=fetch_k)
